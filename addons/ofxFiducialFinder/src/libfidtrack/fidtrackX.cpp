...
static void set_depth( fidRegion *r, short depth );
...
static void sum_leaf_centers( FidtrackerX *ft, fidRegion *r, int width, int height )
{
    int i;
	...
	ft->total_leaf_count ++;
        ...
        for( i=0; i < r->adjacent_region_count; ++i ){
            fidRegion *adjacent = r->adjacent_regions[i];
...
static int check_leaf_variation( FidtrackerX *ft, fidRegion *r, int width, int height )
{
	int i;
...
{
    fidRegion **aa = (fidRegion**)a;
    fidRegion **bb = (fidRegion**)b;
...
static char *build_left_heavy_depth_string( FidtrackerX *ft, fidRegion *r )
{
    int i;
    char *result;
    fidRegion *adjacent;
...
//mk    assert( ft->next_depth_string < ft->depth_string_count );
    ...
    if( r->adjacent_region_count != 1 ){
        ...
        qsort( r->adjacent_regions, r->adjacent_region_count, sizeof(fidRegion*), depth_string_cmp ); 
...
static void print_unordered_depth_string( fidRegion *r )
{
    int i;
    fidRegion *adjacent;
...
static void compute_fiducial_statistics( FidtrackerX *ft, FiducialX *f,
        fidRegion *r, int width, int height )
{
    double all_x, all_y;
...
static void propagate_descendent_count_and_max_depth_upwards(
        Segmenter *s, fidRegion *r, FidtrackerX *ft)
{
    int i;
    fidRegion *parent = 0;
        ...
        FidtrackerX *ft, Segmenter *segments, int width, int height)
{
	...
	for( i=0; i < segments->region_count; ++i ) {
		fidRegion *r = LOOKUP_SEGMENTER_REGION( segments, i );
