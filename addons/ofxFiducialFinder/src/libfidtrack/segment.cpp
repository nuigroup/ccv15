...
void initialize_head_region( fidRegion *r )
{
    r->next = r->previous = r;
}


void link_region( fidRegion *head, fidRegion* r )
{
...
void unlink_region( fidRegion* r )
{
...
#ifndef NDEBUG
static int r1_adjacent_contains_r2( fidRegion* r1, fidRegion* r2 )
{
    int i;

    for( i=0; i < r1->adjacent_region_count; ++i ){
        fidRegion *adjacent = r1->adjacent_regions[i];
        if( adjacent == r2 )
            return 1;
    }
    return 0;
...
static int is_adjacent( fidRegion* r1, fidRegion* r2 )
{
/*
    int i, adjacent_region_count;
    fidRegion **adjacent_regions, *r;
...
static void remove_adjacent_from( fidRegion *r1, fidRegion *r2 ) // remove r2 from r1
{
    int i;
#ifndef NDEBUG
    int found = 0;
...
static void make_saturated( fidRegion* r1 )
{
    int i;
    ...
    for( i = 0; i < r1_adjacent_region_count; ++i ){
        fidRegion *a = r1->adjacent_regions[i];
...
/*
static void make_fragmented( fidRegion* r1 )
{
    int i;
    int r1_adjacent_region_count = r1->adjacent_region_count; 
...
static void make_adjacent( Segmenter *s, fidRegion* r1, fidRegion* r2 )
{
...
static void replace_adjacent( fidRegion *r1, fidRegion *r2, fidRegion *r3 )  // replace r2 with r3 in the adjacency list of r1
{
    int i;
#ifndef NDEBUG
    int found = 0;
...
static void merge_regions( Segmenter *s, fidRegion* r1, fidRegion* r2 )
{
    int i;
    ...
    if( r1->flags & SATURATED_REGION_FLAG ){
        ...
        for( i = 0; i<r2->adjacent_region_count; ++i ){
            fidRegion *a = r2->adjacent_regions[i];
            if( is_adjacent( a, r1 ) ){
...
void initialize_segmenter( Segmenter *s, int width, int height, int max_adjacent_regions )
{
    ...
    s->region_ref_count = 0;
    s->sizeof_region = sizeof(fidRegion) + sizeof(fidRegion*) * (max_adjacent_regions-1);
