...
typedef struct fidRegion{
    struct fidRegion *previous, *next;
    ...
    struct fidRegion *adjacent_regions[ 1 ];   /* variable length array of length max_adjacent_regions */
} fidRegion;
...
#define LOOKUP_SEGMENTER_REGION( s, index )\
    (fidRegion*)(s->regions + (s->sizeof_region * (index)))
