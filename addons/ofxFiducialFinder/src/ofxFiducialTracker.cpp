...
ofxFiducialTracker::ofxFiducialTracker() {
	...
	initTree(ofToDataPath("fiducials/default.txt").c_str());//put trees file in the data folder
	//initTree("fiducials/default.txt");//uncomment this to put trees file in the executable directory
...
//initialize the fiducial tree from fiducials/default.txt file
void ofxFiducialTracker::initTree( const char *file_name ) {
	...
	if(treeidmap.max_adjacencies<=0) {
		...
		treeFromFile = true;
	//	printf("Community Core Vision 1.5 - Initialized - Fidicual Tree Map: '%s'.\n", file_name);
		printf("Community Core Visin (MC) 1.5 Initialized - http://ccv.nuigroup.com/1.5/");
	}
}
